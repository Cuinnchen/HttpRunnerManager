...
def edit_case(request, id=None):
    ...
    if request.session.get('login_status'):
        ...
        request_value = request_data['request']
        logger.info(request_value['data'])
        if isinstance(request_value['data'], str):
            logger.info("request_value['data'] is str")
        if 'data' in request_value.keys() and isinstance(request_value['data'], str):
            data_body = request_value['data']
            del request_value['data']
            request_value.setdefault('xml', {'body': data_body})
